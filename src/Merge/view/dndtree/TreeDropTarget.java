...
/**
 * <p>
 * A drop target for class <code>JTree</code> that implements autoscrolling, automatic tree node expansion and a custom drag image during
 * the drag part of a drag and drop operation.
 * </p>
 * 
 ...
 * @author published under the terms and conditions of the GNU General Public License, for details see file in the distribution package of
 *         this software
 * 
...
public class TreeDropTarget extends DropTarget {
      ...
      * 
      * @param the
      *             transfer handler that provides the drag image for the currently dragged node
      */
     ...
     public synchronized void dragEnter(DropTargetDragEvent dtde) {
          if (!rejectOrAcceptDrag(dtde))
               super.dragEnter(dtde);
     }
     
     
     private boolean rejectOrAcceptDrag(DropTargetDragEvent dtde) {
          JTree tree = (JTree) dtde.getDropTargetContext().getComponent();
          Point loc = dtde.getLocation();         
          int row = tree.getRowForPath(tree.getClosestPathForLocation(loc.x, loc.y));
     ...
     /**
      * use method dragOver to constantly update the drag mark and darg image as well as to support automatic scrolling durng a drag
      * operation
      */
     ...
     /**
      * scroll visible tree parts when user drags outside an 'inner part' of the visible region
      */
     private void autoscroll(JTree tree, Point cursorLocation) {
          ...
          Rectangle inner = new Rectangle(outer.x + insets.left, outer.y
                    + insets.top, outer.width - (insets.left + insets.right), outer.height
     ...
     public void updateDragMark(JTree tree, Point location) {
          ...
          if (path != null) {
               ...
               /*
                * find out if we have to mark a tree node or if we have to draw an insertion marker
                */
               int rby = rowBounds.y;
               int topBottomDist = insertAreaHeight / 2;
               // x = top, y = bottom of insert area
               Point topBottom = new Point(rby - topBottomDist, rby
                         + topBottomDist);
     ...
     private void markNode(JTree tree, Point location) {
          ...
          if (path != null) {
               DefaultMutableTreeNode treeNode = (DefaultMutableTreeNode) path.getLastPathComponent();
               Node node = (Node) treeNode.getUserObject();
               if (!node.isRemoved() && node.getType() == NodeConstants.TYPE_ELEMENT) {
                    if (lastRowBounds != null) {
                         Graphics g = tree.getGraphics();
                         g.setColor(Color.white);
                         g.drawLine(lastRowBounds.x, lastRowBounds.y, lastRowBounds.x
     ...
     private void paintInsertMarker(JTree tree, Point location) {
          ...
          if (path != null) {
               ...
               if (rowBounds != null) {
                    ...
                    g.drawLine(rowBounds.x, rowBounds.y, rowBounds.x
                              + rowBounds.width, rowBounds.y);
               }
